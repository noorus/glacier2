...
static FORCEINLINE NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *CallMalloc(void *RESTRICT mspace, size_t size, size_t alignment, unsigned flags) THROWSPEC
{
	...
	ret=mspace_malloc2((g_dl::mstate) mspace, size, alignment, flags);
#ifdef HAVE_VALGRIND
	...
	if(ret)
	{
		g_dl::mchunkptr p=mem2chunk(ret);
		size_t truesize=chunksize(p) - overhead_for(p);
		if(!leastusedaddress || (void *)((g_dl::mstate) mspace)->least_addr<leastusedaddress) leastusedaddress=(void *)((g_dl::mstate) mspace)->least_addr;
		if(!largestusedblock || truesize>largestusedblock) largestusedblock=(truesize+g_dl::mparams.page_size) & ~(g_dl::mparams.page_size-1);
	}
#endif
#endif
	if(!ret) return 0;
...
#elif USE_ALLOCATOR==1
	ret=mspace_realloc2((g_dl::mstate) mspace, mem, newsize, alignment, flags);
#ifdef HAVE_VALGRIND
...
#else
	mspace_free((g_dl::mstate) mspace, mem);
#endif
#endif
}

static NEDMALLOCNOALIASATTR g_dl::mstate nedblkmstate(void *RESTRICT mem) THROWSPEC
{
	if(mem)
	{
#if USE_MAGIC_HEADERS
...
#pragma pack(pop)
		...
		when the preceding block is allocated because ptmalloc2 finds the local g_dl::mstate by rounding the ptr
...
#warning nedmalloc is unreliable with foreign memory blocks, so make sure you really want ENABLE_TOLERANT_NEDMALLOC turned on!
#endif
		{
			...
			mchunkptr->prev_foot = mem-(2*size_t) = g_dl::mstate ^ mparams.magic for PRECEDING block;
			...
			*/
			...
			{
				g_dl::mchunkptr p=mem2chunk(mem);
				g_dl::mstate fm=0;
...
struct nedpool_t
{
	...
	g_dl::mstate m[MAXTHREADSINPOOL+1];		/* mspace entries for this pool */
};
...
#elif USE_ALLOCATOR==1
	if(!(p->m[0]=(g_dl::mstate) g_dl::create_mspace(capacity, 1))) goto err;
#ifdef HAVE_VALGRIND
...
static NOINLINE g_dl::mstate FindMSpace(nedpool *RESTRICT p, threadcache *RESTRICT tc, int *RESTRICT lastUsed, size_t size) THROWSPEC
{	/* Gets called when thread's last used mspace is in use. The strategy
	...
	if(end<p->threads)
	{
...
#elif USE_ALLOCATOR==1
		if(!(temp=(g_dl::mstate) g_dl::create_mspace(size, 1)))
			goto badexit;
#endif
		...
		{	/* Drat, must destroy it now */
			RELEASE_LOCK(&p->mutex);
#if USE_ALLOCATOR==1
			g_dl::destroy_mspace((g_dl::mstate) temp);
#endif
			goto badexit;
		}
		...
		{
			volatile struct g_dl::malloc_state **_m=(volatile struct g_dl::malloc_state **) &p->m[end];
			*_m=(p->m[end]=temp);
		}
...
void neddestroysyspool() THROWSPEC
{
	...
	for(n=0; n<MAXTHREADSINPOOL+1; n++)
		p->m[n]=(g_dl::mstate)(size_t)(sizeof(size_t)>4 ? 0xdeadbeefdeadbeefULL : 0xdeadbeefUL);
	if(TLSFREE(p->mycache)) abort();
#if USE_LOCKS
	RELEASE_LOCK(&p->mutex);
	DESTROY_LOCK(&p->mutex);
...
{
	nedpool *np=0;
	g_dl::mstate fm=nedblkmstate(mem);
...
#define GETMSPACE(m,p,tc,ms,s,action)                 \
  do                                                  \
  {                                                   \
    g_dl::mstate m = GetMSpace((p),(tc),(ms),(s));          \
    action;                                           \
...
static FORCEINLINE g_dl::mstate GetMSpace(nedpool *RESTRICT p, threadcache *RESTRICT tc, int mymspace, size_t size) THROWSPEC
{	/* Returns a locked and ready for use mspace */
	g_dl::mstate m=p->m[mymspace];
	assert(m);
#if USE_LOCKS && USE_ALLOCATOR==1
...
struct nedmallinfo nedpmallinfo(nedpool *p) THROWSPEC
{
	...
	for(n=0; p->m[n]; n++)
	{
		...
		struct g_dl::mallinfo t=g_dl::mspace_mallinfo(p->m[n]);
		ret.arena+=t.arena;
...
{
#if USE_ALLOCATOR==1
	return g_dl::mspace_mallopt(parno, value);
#else
	return 0;
#endif
}
...
{
#if USE_ALLOCATOR==1
	if(granularity) *granularity=g_dl::mparams.granularity;
	if(magic) *magic=g_dl::mparams.magic;
	return (void *) &syspool;
#else
...
int    nedpmalloc_trim(nedpool *p, size_t pad) THROWSPEC
{
	...
	{
#if USE_ALLOCATOR==1
		ret+=g_dl::mspace_trim(p->m[n], pad);
#endif
	}
	return ret;
}
void   nedpmalloc_stats(nedpool *p) THROWSPEC
{
	...
	{
#if USE_ALLOCATOR==1
		g_dl::mspace_malloc_stats(p->m[n]);
#endif
	}
}
size_t nedpmalloc_footprint(nedpool *p) THROWSPEC
{
	...
	{
#if USE_ALLOCATOR==1
		ret+=g_dl::mspace_footprint(p->m[n]);
#endif
	}
	return ret;
}
static FORCEINLINE NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void ** CallIndependentCalloc(void *RESTRICT m, size_t elemsno, size_t elemsize, void **chunks) THROWSPEC
{
...
#elif USE_ALLOCATOR==1
    ret=g_dl::mspace_independent_calloc(m, elemsno, elemsize, chunks);
#ifdef HAVE_VALGRIND
    if(ret)
    {
        size_t n;
    ...
    if(ret)
    {
  		if(!leastusedaddress || (void *)((g_dl::mstate) m)->least_addr<leastusedaddress) leastusedaddress=(void *)((g_dl::mstate) m)->least_addr;
...
#elif USE_ALLOCATOR==1
	ret=g_dl::mspace_independent_comalloc(m, elems, sizes, chunks);
#ifdef HAVE_VALGRIND
	if(ret)
	{
		size_t n;
